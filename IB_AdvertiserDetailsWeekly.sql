...
(
	...
	where ((cal1.calendar_date >= '2015-11-26' and cal1.calendar_date <='2016-02-09') or  cal1.calendar_date ='3000-12-31')
) as ch
left outer join 
(
	...
	,count(distinct case when GlobalFirstDepositDate is not null then PlayerId end ) as FirstTimeDepositors
...
(
	select coalesce(AdvCode,'NA') AdvCode,coalesce(AdvUsername,'NA') AdvUsername,coalesce(AdvEmail,'NA') AdvEmail,AdvChannel
	,date(coalesce(p.GlobalFirstDepositDate,'3000-12-31')) ftd	
	,count(distinct case when SignupDate is not null then PlayerId end ) as UniqueSignUps
	from romaniamain.dim_calendar as cal
...
left outer join 
(
    select AdvCode,AdvUsername,AdvEmail,AdvChannel,ftd,count(distinct playerid) UAP, 
    sum(SystemAPD) SystemAPD, sum(SportsAPD) SportsAPD, sum(EGamingAPD) EGamingAPD
    from (
   	SELECT dps.playerid,
    coalesce(AdvCode,'NA') AdvCode,coalesce(AdvUsername,'NA') AdvUsername,coalesce(AdvEmail,'NA') AdvEmail,AdvChannel
	,date(coalesce(p.GlobalFirstDepositDate,'3000-12-31')) ftd
	,COUNT(distinct case when (dps.SPStakeAmt+dps.EGBet) > 0 then dps.SummaryDate end) as SystemAPD
	,COUNT(distinct case when (dps.SPStakeAmt) > 0 then dps.SummaryDate end) as SportsAPD
	,COUNT(distinct case when (dps.EGBet) > 0 then dps.SummaryDate end) as EGamingAPD
	FROM 
	romaniamain.sd_gv_daily_player_summary as dps
	join romaniafl.Dim_Player_Channel as p on dps.PlayerId = p.PlayerId
	where p.SignupDate >= '2015-11-26'
	group by 1,2,3,4,5,6) as temp    
    group by 1,2,3,4,5
...
where 
((ch.calendar_date >= '2015-11-26' and ch.calendar_date <= '2016-02-09') or ch.calendar_date = '3000-12-31')
order by 1,2;
