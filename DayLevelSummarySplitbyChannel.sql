...
from 
(select 
calendar_date, ClientType, Platform, AdvChannel
from romaniamain.dim_calendar 
cross join (select coalesce(SignupClientType,'NA') ClientType,coalesce(SignupClientPlatform,'NA') Platform,AdvChannel 
from romaniafl.dim_player_channel group by 1,2,3) ch) as cal
left outer join 
(
	...
	group by 1,2,3,4
) as FTD on cal.calendar_date = FTD.SummaryDate and cal.ClientType = ftd.ClientType and cal.Platform = ftd.Platform and cal.AdvChannel = ftd.AdvChannel
left outer join 
(
	select cal.calendar_date as SummaryDate,
	coalesce(SignupClientType,'NA') ClientType,
	...
	where cal.calendar_date >= '2015-11-26' and cal.calendar_date <= '2016-02-14'
	group by 1,2,3,4
) as SignUp on cal.calendar_date = SignUp.SummaryDate and cal.ClientType = SignUp.ClientType and cal.Platform = SignUp.Platform and cal.AdvChannel = SignUp.AdvChannel
left outer join 
(
	...
	group by 1,2,3,4
) as Cashier on cal.calendar_date = Cashier.SummaryDate and cal.ClientType = Cashier.ClientType and cal.Platform = Cashier.Platform and cal.AdvChannel = Cashier.AdvChannel
left outer join 
(
	...
	where p.SignupDate >= '2015-11-26'
	group by 1,2,3,4
) as GameView on cal.calendar_date = GameView.SummaryDate and cal.ClientType = GameView.ClientType and cal.Platform = GameView.Platform and cal.AdvChannel = GameView.AdvChannel
left outer join
(
	SELECT dps.SummaryDate
	,coalesce(SignupClientType,'NA') ClientType
	...
	where p.SignupDate >= '2015-11-26'
	group by 1,2,3,4
) as CashView on cal.calendar_date = CashView.SummaryDate and cal.ClientType = CashView.ClientType and cal.Platform = CashView.Platform and cal.AdvChannel = CashView.AdvChannel
left outer join
(
	SELECT cal.calendar_date as SummaryDate
	,coalesce(SignupClientType,'NA') ClientType
	...
	where p.SignupDate >= '2015-11-26'
	group by 1,2,3,4
) as Cumulative on cal.calendar_date = Cumulative.SummaryDate and cal.ClientType = Cumulative.ClientType and cal.Platform = Cumulative.Platform and cal.AdvChannel = Cumulative.AdvChannel
left outer join
(
	select SummaryDate
	,coalesce(SignupClientType,'NA') ClientType
	...
	group by 1,2,3,4
) as Balance on cal.calendar_date = Balance.SummaryDate and cal.ClientType = Balance.ClientType and cal.Platform = Balance.Platform and cal.AdvChannel = Balance.AdvChannel
left outer join
(
	...
	group by 1,2,3,4
) as TokenPL on cal.calendar_date = TokenPL.SummaryDate and cal.ClientType = TokenPL.ClientType and cal.Platform = TokenPL.Platform and cal.AdvChannel = TokenPL.AdvChannel
where 
cal.calendar_date >= '2015-11-26' and cal.calendar_date <= '2016-02-14'
order by 1;
