...
count( distinct case when LiveYN ='Y' then EventId end ) eventCnt 
...
select SummaryDate, LiveYN, BetType, PlayerID, betId, StakePerLineMtplr*TotalStake,StakePerLineMtplr*TotalReturn,EventCnt
...
select SummaryDate, LiveYN, BetType, PlayerID, sum(TotalStake), sum(TotalReturn), count(distinct betId) slipCount,sum(EventCnt) EventCnt
...
sum(TotalStake) TotalBetFixed,
sum(TotalReturn) TotalWinFixed,
count(distinct case when TotalStake > 0 then ab.PlayerID END) "A.U.",
