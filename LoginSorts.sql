use romaniastg;
drop table romaniastg.frm_f_DailyAccntBalSummary;
create table romaniastg.frm_f_DailyAccntBalSummary (
SummaryDate date,
PlayerCode int(10),
...
drop table romaniastg.stg_login_across;
CREATE TABLE romaniastg.stg_login_across (
...
from romaniastg.c_logins1 lo1 where date(lo1.LoginDate) != date(lo1.logoutdate) 
#and playercode=10275515
) sa 
join romaniastg.d_date d on d.dat_Day_Date between date(sa.logindate) and date(sa.logoutdate)
where d.dat_day_date >= '2015-08-01' and d.dat_day_Date < current_date
INTO OUTFILE 'C:\\Users\\CSQ-MARK5-REP-LAYER\\Downloads\\loginMultiplesegregated.csv'
#character set utf8
FIELDS TERMINATED BY ';' 
ENCLOSED BY 'NULL'
LINES TERMINATED BY '\r\n';  

LOAD DATA INFILE 'C:\\Users\\CSQ-MARK5-REP-LAYER\\Downloads\\loginMultiplesegregated.csv' 
INTO TABLE romaniastg.stg_login_across
FIELDS TERMINATED BY ';' 
ENCLOSED BY 'NULL'
LINES TERMINATED BY '\r\n';
...
from romaniastg.c_logins1 lo
join (select playercode,date(logindate) date,min(logindate) logindate
from romaniastg.c_logins1 lo where date(lo.LoginDate) = date(lo.logoutdate) 
...
INTO OUTFILE 'C:\\Users\\CSQ-MARK5-REP-LAYER\\Downloads\\loginSingle.csv'
#character set utf8
FIELDS TERMINATED BY ';' 
ENCLOSED BY 'NULL'
LINES TERMINATED BY '\r\n';

LOAD DATA INFILE 'C:\\Users\\CSQ-MARK5-REP-LAYER\\Downloads\\loginSingle.csv' 
INTO TABLE romaniastg.stg_login_across
FIELDS TERMINATED BY ';' 
ENCLOSED BY 'NULL'
LINES TERMINATED BY '\r\n';
...
from 
(select lo.LoginDate,lo.PlayerCode,lo.PlayerType,lo.Startbalance,lo.Startbonusbalance
from romaniastg.stg_login_across lo
join (select lo1.playercode,date(lo1.logindate) date,min(lo1.logindate) logindate
from romaniastg.stg_login_across lo1 where date(lo1.LoginDate) = date(lo1.logoutdate) 
group by playercode,date(logindate)) st on lo.playercode = st.playercode and lo.logindate = st.logindate) st1
join
(select lo.LoginDate,lo.LogoutDate,lo.PlayerCode,lo.PlayerType, lo.EndBalance,lo.EndBonusBalance 
from romaniastg.stg_login_across lo
...
group by playercode,date(logindate)) en on lo.playercode = en.playercode and lo.logoutdate = en.logoutdate)en1
on date(st1.LoginDate) = date(en1.LoginDate) and st1.PlayerCode = en1.PlayerCode
INTO OUTFILE 'C:\\Users\\CSQ-MARK5-REP-LAYER\\Downloads\\frm_f_DailyAccntBalSummary.csv'
#character set utf8
FIELDS TERMINATED BY ';' 
ENCLOSED BY 'NULL'
LINES TERMINATED BY '\r\n';

LOAD DATA INFILE 'C:\\Users\\CSQ-MARK5-REP-LAYER\\Downloads\\frm_f_DailyAccntBalSummary.csv' 
INTO TABLE romaniastg.frm_f_DailyAccntBalSummary
FIELDS TERMINATED BY ';' 
ENCLOSED BY 'NULL'
LINES TERMINATED BY '\r\n';


select * from romaniastg.frm_f_DailyAccntBalSummary where playercode=10275515